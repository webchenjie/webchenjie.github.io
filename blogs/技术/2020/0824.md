...
## 开发手册
    ...
    56. 一般来说，行内元素是不可以设置宽高的，如果想给行内元素设置宽高，则需要将其display设置为block/inline-block/flex/inline-flex
       （使用position:absolute/fixed或float：left/right，在computed中查看display会发现属性值也为block，所以这两个方法用的原理也是这个）
    57. 在微信开发者工具中获取企微环境第一次没问题，第二次开始就获取不到
    58. uni.getStorage 如果 success 和 fail 是异步函数，complete 有时会不执行
    59. 路由传参时要通过 encodeURI 或 encodeURIComponent 进行参数处理，如果涉及对象时需要 JSON.stringify 处理，
        解析的时候通过 decodeURI 或 decodeURIComponent 进行处理，如果涉及对象时需要 JSON.parse 处理
    60. 后端一次性返回 10w 条数据，该如何处理
        1. 后端的问题，首先要用后端的思维去解决——中间层，前端写 node 中间件处理拆分
        2. 虚拟列表（只渲染可视区），注意实现复杂而效果不一定好（低配手机）
        3. 一般不会这样返回，JS 没问题，但是渲染到 DOM 会非常卡顿
    61. 文本溢出省略号显示，在 iOS 和 Android 的展示效果不一样，特别是在小程序中，iOS 会靠底部显示，Android 的省略号垂直居中
    62. 可以使用 edge 或 chrome 在 url 上输入 //inspect/#devices 配合手机使用对应的浏览器打开对应的调试页面
        这样就可以在手机上进行真机调试
    63. 分片上传：
    ...
    64. axios 的拦截器可以设置多个，请求拦截器如果存在多个，后面添加的会先执行，响应拦截器如果存在多个，前面添加的会先执行
        请求拦截器会返回一个 id，可以调用 eject 删除对应 id 的请求拦截器
    65. js 可以通过 FileReader 对象读取文件，并可转换为 base64、二进制码、文本
    66. js 可以将 base64 转为 File 对象
