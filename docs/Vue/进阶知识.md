...
10. Dep 和 Watcher
    ...
    2. Watcher是Vue中的观察者，它用于监听数据的变化，并在数据变化时执行相应的回调函数。
        每个Vue组件实例都有一个对应的Watcher实例，用于记录该组件依赖的数据属性
        有 data 类型、computed 类型、watch 类型
        class Watcher {
...
12. 它的流程是：
    ...
    3. updateChildren:
        ...
        如果没有找到,新的子节点进行创建并插入,如果找到了,这两个节点再进行vnode比较
        也可以简单的理解为,如果没有是重新渲染,如果有的话,直接把旧的子节点挪过来用就可以了
        1. 双端比较（oldVnode 和 newVnode）
            1. 开始和开始对比，有命中则进行 patchVnode，然后 index 向中间靠拢，小的++，大的--
            2. 结束和结束对比，有命中则进行 patchVnode，然后 index 向中间靠拢，小的++，大的--
            3. 开始和结束对比，有命中则进行 patchVnode，然后 index 向中间靠拢，小的++，大的--
            4. 结束和开始对比，有命中则进行 patchVnode，然后 index 向中间靠拢，小的++，大的--
            5. 如果以上都没命中，则拿新节点的 key 和旧节点的 key 进行匹配，看是否能找到
                如果找不到，那就新建一个节点进行插入
                如果找到了，还要判断 el 是否相等，如果不相等则新建一个节点进行插入
                如果相等则进行 patchVnode，然后 index 向中间靠拢，小的++，大的--
----------------------------------------------------------------------------------------------
