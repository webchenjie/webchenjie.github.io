...
3. 可以不用bind(this)吗？
  ...
  2. 在 JSX 模板中直接绑定事件处理函数名称，在函数定义的时候使用箭头函数 // 此时也是不能够传递参数
...
1. 当组件的 state 或 props 发送改变的时候，render 函数就会重新执行
...
4. batchUpdate 机制 // 同步还是异步
  1. 在 react 组件中的函数执行时，如果有调用了 setState 方法，那么这个函数在执行前 react 内部会
     有一个 isBatchingUpdates 被设置为 true，当函数执行完后这个 isBatchingUpdates 会被设置成 false，如果是 true 就会走到
     batchUpdate 机制中，就是异步更新，如果是 false，就不会走到，就是同步更新，而上面写到在 setTimeout
     和自定义事件是同步更新的原因就是因为这两个代码是异步执行的，所以函数已经同步执行完了，isBatchingUpdates 已经是 false 了
     自然就不会走到 batchUpdate 机制中，就会走同步更新！！！
  2. 当 isBatchingUpdates 是 true 时，会把当前组件保存到 dirtyComponents 中，当 isBatchingUpdates 是 false 时
     会遍历所有的 dirtyComponents 调用 updateComponent 再更新 pending state or props
...
----------------------------------------------------------------------------------------------
// setState 可能会被合并
1. 如果 setState 传入的是对象就会被合并，如果是函数则不会
  1. 这里连续直接两遍，count 初始值是 0，执行完 count 是 1，而不是 2
    this.setState({
...
4. 示例
  ...
  const ThemeContext = React.createContext('light') // light 只是默认值
  ...
  sunzi1.contextType = ThemeContext // ThemeContext 就是爷爷组件的定义的
