...
3. 示例父组件 TodoList
  ...
  class TodoList extends Component {
    ...
    handleInputChange(e) {
      ...
      const value = e.target.value // 必须在外面定义，否则报错
      this.setState(() => {
        return {
          inputValue: value
        }
    ...
    handleBtnClick() {
      ...
      // 在官网 react 中推荐使用传入函数而不是对象，接收的参数是改变之前的 state
...
3. 原因就是 setState 是异步的
4. 可以在 setState 方法中传入第二个参数，它是一个函数，写在函数体中就可以获取到更新完数据的 DOM 节点
5. 示例
import React, { Component } from 'react' // 一定要引入 React 否则编译报错，虽然在代码没显式用到
  class TodoItem extends Component {
    constructor(props) {
