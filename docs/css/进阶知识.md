...
```js
// 盒模型
1. box-sizing: border-box // offsetwidth = (内容宽度 + 内边距 + 边框),无外边框
----------------------------------------------------------------------------------------------
// margin
1. margin纵向重叠:
...
3. 圣杯布局和双飞翼布局的技术总结
    使用float布局
    两侧使用margin负值,以使和中间内容横向重叠
    防止中间内容被两侧覆盖
    圣杯布局用padding, 留出两侧的空间, 左边使用 margin-left: 100% + right: 宽度, 右边使用 margin-right: 宽度
    双飞翼用margin, 留出两侧的空间, 左边使用 margin-left: 100%, 右边使用 margin-left: 宽度
----------------------------------------------------------------------------------------------
// 居中方法
1. 水平居中:
...
2. line-height继承
    ...
    写百分比,如200%则继承计算出来的值,父级计算
3. 如果div不设置高度时,div的高度是由line-height控制的
4. line-height比font-size高一个行距,上下各一半
----------------------------------------------------------------------------------------------
// rem
1. px是绝对长度单位,最常用
2. em是相对长度单位,相对父元素,不常用
3. rem是相对长度单位,相对根元素,用于响应式布局
