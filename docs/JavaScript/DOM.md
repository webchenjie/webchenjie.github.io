...
1. property 修改对象属性，不会体现到 HTML 结构中 // 优先使用
...
2. ele.insertBefore(ele, 子节点) // 为ele节点的前面一个子节点添加节点,返回新的子节点,如果参数是已存在的节点则会移动
...
3. scrollWidth = content + padding-left + padding-right + border-left + border-right + scrollbar + 滚动进入不可见的内容
----------------------------------------------------------------------------------------------
// 事件
