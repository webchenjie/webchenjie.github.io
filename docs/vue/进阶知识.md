...
8. 示例:
    ...
    _v: createTextVnode
    _s: toString
9. 在vue组件中可以直接写render函数代替template
...
----------------------------------------------------------------------------------------------
// 依赖收集
1. 同时每一个 Vue 组件实例，都对应着一个 watcher 实例；这个 watcher 实例仿佛一个跟踪狂，它的目光永远跟随着 data
2. 由于 render 函数的执行依赖于数据的读取，因此渲染时必定会读取 data 属性进而触发其对应的 getter 方法
3. getter 方法被调用后，会通知到 watcher，watcher 就会把这些 getter 方法被触发的属性记录为“依赖”——这一过程就是大家常常听到的“依赖收集”过程
4. 如果 data 发生了更新，也就是说被“写”了，此时对应属性的 setter 方法就会被触发。setter 也会去通知 watcher，告诉它“我改变了”
5. watcher 拿到消息后，立刻跑去告诉 render：“data 变了，你也给我跟着变！”。由此去触发一个 re-render 的过程、与数据更新相关的组件会重新渲染。
----------------------------------------------------------------------------------------------
1. Observer：处理 data 的家伙。它会给 data 安装 getter 和 setter，这些安装上的逻辑会联动 Dep 去完成依赖收集和更新的派发
2. Dep：实际通知 Watcher 的人。在 getter 和 setter 逻辑中，正是通过调度 Dep 来完成信息的收集、以及和 Watcher 间的通信
3. Watcher：Watcher 被通知之后，就会通知 render、进而触发重渲染了
4. 总结
  1. Observer 就是通过遍历给每个变量通过 Object.defineProperty() 实现数据劫持
  2. 当某一个变量发生了 getter 会在 Dep 中增加对应的 watcher，发生了 setter 会在 Dep 中遍历通知对应的 watcher
  3. 一个变量对应多个 watcher，userWatcher、computedWatcher、watchWatcher
  4. watcher 再根据通知进行处理，如 computedWatcher 和 watchWatcher 就执行开发者自定义的回调方法
----------------------------------------------------------------------------------------------
// vuex
