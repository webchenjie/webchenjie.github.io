...
5. toRefs 处理响应式对象解构时的响应式数据中存在的值 // 用于响应式对象中的变量实现响应式
  const app = Vue.createApp({
    template: `
      <div>{{ name }}</div>
    `,
    setup(props, context) {
      ...
      return { name } // 此时导出的是 name，而不是 nameObj，这时 name 不是响应式，需要使用 toRefs
    }
  })
6. toRef 处理响应式对象解构时的响应式数据中不存在的值 // 用于响应式对象中的变量实现响应式
  const app = Vue.createApp({
    template: `
      <div>{{ age}}</div>
    `,
...
15. vue-router
  import { useRputer } from 'vue-router'
  const router = useRputer()
...
8. proxy 如何实现响应式？
  ...
  3. 深度监听，性能更好， vue2.x 的响应式需要一次性递归，而 vue3 则是使用了才需要递归
  4. 可监听新增或删除的属性
  5. 可监听数组变化
...
15. Composition API 和 React Hooks 对比
  ...
  4. 前者 reactive + ref 比后者 useState 要更能理解
```
